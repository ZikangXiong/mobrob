...
COLOR_WALL = np.array([0.5, 0.5, 0.5, 1])
...
class Engine(gym.Env, gym.utils.EzPickle):
    ...
    def build_observation_space(self):
        ...
        if self.observe_qvel:
            obs_space_dict["qvel"] = gym.spaces.Box(
                -np.inf, np.inf, (self.robot.nv,), dtype=np.float32
            )
        if self.observe_ctrl:
            obs_space_dict["ctrl"] = gym.spaces.Box(
                -np.inf, np.inf, (self.robot.nu,), dtype=np.float32
            )
    ...
    def draw_placement(self, placements, keepout):
        ...
        if placements is None:
            ...
            assert len(
                constrained
            ), "Failed to find any placements with satisfy keepout"
        ...
        if self.gremlins_num:
            ...
            for i in range(self.gremlins_num):
                ...
                object = {
                    ...
                    "pos": np.r_[
                        self.layout[name.replace("obj", "")], self.gremlins_size
                    ],
        ...
        if self.walls_num:
            for i in range(self.walls_num):
                ...
                if isinstance(self.walls_size, int):
                    ...
                    raise ValueError(
                        f"walls_size must be int or list, not {type(self.walls_size)}"
                    )
        ...
        if self.gremlins_num:
            for i in range(self.gremlins_num):
                ...
                mocap = {
                    ...
                    "type": "box",
                    "pos": np.r_[
                        self.layout[name.replace("mocap", "")], self.gremlins_size
                    ],
                    "rot": self._gremlins_rots[i],
                    "group": GROUP_GREMLIN,
                    "rgba": np.array([1, 1, 1, 0.1]) * COLOR_GREMLIN,
                }
    ...
    def obs(self):
        ...
        assert self.observation_space.contains(
            obs
        ), f"Bad obs {obs} {self.observation_space}"
        return obs
    ...
    def step(self, action):
        ...
        exception = False
        for _ in range(
            self.rs.binomial(self.frameskip_binom_n, self.frameskip_binom_p)
        ):
            try:
                self.set_mocaps()
    ...
    def render(
        ...
        if vertical:
            ...
            self.viewer.cam.azimuth = (
                90  # camera rotation around the camera's vertical axis
            )
        ...
        if self._cost.get("cost", 0) > 0:
            self.render_sphere(self.world.robot_pos(), 0.25, COLOR_RED, alpha=0.5)
