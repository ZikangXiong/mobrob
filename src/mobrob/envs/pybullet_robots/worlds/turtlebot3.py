...
def create_wall(wall_length,
                wall_thick,
                wall_height,
                client_id):
    half_extents, frame_positions = wall_info(wall_length, wall_thick, wall_height)

    wall_vid = p.createVisualShapeArray(shapeTypes=[p.GEOM_BOX] * 4,
                                        halfExtents=half_extents,
                                        rgbaColors=[[0.9, 0.6, 0.6, 1]] * 4,
                                        visualFramePositions=frame_positions,
                                        physicsClientId=client_id)
    wall_cid = p.createCollisionShapeArray(shapeTypes=[p.GEOM_BOX] * 4,
                                           halfExtents=half_extents,
                                           collisionFramePositions=frame_positions,
                                           physicsClientId=client_id)
    wall_id = p.createMultiBody(baseVisualShapeIndex=wall_vid,
...
class World(WorldBase):
    ...
    def _build_world(self):
        ...
        create_wall(wall_length=2.42 + 0.28 * 2,
                    wall_thick=0.265,
                    wall_height=0.265,
